...
Nightwatch.prototype.terminate = function(deferred, force) {
  ...
  if (this.options.single_session && global.modulePaths.length && !force) {
    return this;
  }

  if (deferred) {
    this.queue.instance().once('queue:started', this.terminateSession.bind(this,force));
  } else {
    this.terminateSession(force);
  }
...
Nightwatch.prototype.terminateSession = function(force) {
  this.queue.reset();
  this.queue.empty();
...
Nightwatch.prototype.startSession = function () {
  ...
  process.on('SIGINT', function(){
    self.on('nightwatch:finished', function(){process.exit(0)});
    self.terminate(null,true);
  })
