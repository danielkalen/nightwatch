...
class Runner extends require('events') {
  constructor(testSource, opts, additionalOpts, doneCb){
    super()
...
Runner.prototype.addTestModule = function(modulePath, fullPaths) {
  try {
    currentTestSuite = this.currentTestSuite = new TestSuite(modulePath, fullPaths, this.options, this.additionalOpts, this.doneCb, this);
  } catch (err) {
    this.doneCb(err);
    return null;
  }
 return this.runTestModule(currentTestSuite);
}

Runner.prototype.runTestModule = function(currentTestSuite) {
  var moduleKey = currentTestSuite.getReportKey();
  ...
  this.globalResults.modules[moduleKey] = {
    ...
    time : null,
    timestamp : null,
    group : currentTestSuite.getGroupName()
  };
...
Runner.prototype.run = function runner() {
  ...
  Runner.readPaths(this.testSource, this.options)
    .spread(function(modulePaths, fullPaths) {
      global.modulePaths = modulePaths;
      (function runNextModule() {
        var modulePath = modulePaths.shift();
        var promise = self.addTestModule(modulePath, fullPaths);
